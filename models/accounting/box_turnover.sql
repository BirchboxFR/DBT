WITH all_box_sales AS (
  ...
  INNER JOIN inter.order_detail_sub s ON br.sub_id = s.id AND br.dw_country_code = s.dw_country_code
  LEFT JOIN {{ ref('box_sales') }} bs ON bs.sub_id = s.id AND br.dw_country_code = bs.dw_country_code
  LEFT JOIN `payment.adyen_notifications_authorization` ana ON ana.sub_id = br.sub_id AND ana.dw_country_code = br.dw_country_code
