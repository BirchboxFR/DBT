...
FROM inter.comments co
LEFT JOIN inter.products p ON p.post_id = co.comment_post_id AND p.dw_country_code = co.dw_country_code
LEFT JOIN {{ ref('catalog') }} ca ON ca.product_id = p.id AND ca.dw_country_code = p.dw_country_code
LEFT JOIN inter.users c ON c.user_email = co.comment_author_email AND c.dw_country_code = co.dw_country_code
left JOIN inter.posts post ON post.id = p.post_id and post.dw_country_code=p.dw_country_code
