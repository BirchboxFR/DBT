...
(
  ...
  s.coffret_id,
  b.date,
  DATE_DIFF(CASE WHEN an.eventDate IS NULL THEN s.last_payment_date ELSE an.eventDate END, b.shipping_date, DAY) + 1  AS day_in_cycle,
  CASE WHEN an.eventDate IS NULL THEN s.last_payment_date ELSE an.eventDate END AS payment_date,
 DATE_DIFF(CASE WHEN an.eventDate IS NULL THEN s.last_payment_date ELSE an.eventDate END, b.shipping_date, DAY) + 1 AS nb_days_since_opening,
 date_diff(CASE WHEN an.eventDate IS NULL THEN s.last_payment_date ELSE an.eventDate END, b1.shipping_date, DAY) as nb_days_next_cycle,
