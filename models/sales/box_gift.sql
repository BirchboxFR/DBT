...
INNER JOIN inter.order_detail_sub s ON s.dw_country_code = d.dw_country_code AND s.order_detail_id = d.id
INNER JOIN inter.order_detail_sub s1 ON s1.dw_country_code = s.dw_country_code AND s1.order_detail_id = s.order_detail_id AND s1.box_id <= s.box_id
INNER JOIN inter.boxes b ON b.dw_country_code = s.dw_country_code AND b.id = s.box_id
INNER JOIN inter.boxes b1 ON b1.dw_country_code = s1.dw_country_code AND b1.id = s1.box_id
INNER JOIN snippets.current_box cb ON o.dw_country_code = cb.dw_country_code
