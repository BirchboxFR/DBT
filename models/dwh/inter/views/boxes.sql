{%- set fr_columns = adapter.get_columns_in_relation(api.Relation.create(schema='bdd_prod_fr', identifier='wp_jb_boxes')) -%}
{%- set de_columns = adapter.get_columns_in_relation(api.Relation.create(schema='bdd_prod_de', identifier='wp_jb_boxes')) -%}
...
FROM `bdd_prod_fr.wp_jb_boxes` t
...
FROM `bdd_prod_de.wp_jb_boxes` t
...
FROM `bdd_prod_es.wp_jb_boxes` t
...
FROM `bdd_prod_it.wp_jb_boxes` t
