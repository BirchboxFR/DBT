{{ config(
    ...
    cluster_by=['dw_country_code','special_type']
) }}
...
{%- set fr_columns = adapter.get_columns_in_relation(api.Relation.create(schema='bdd_prod_fr', identifier='wp_jb_partial_cancelations')) -%}
{%- set de_columns = adapter.get_columns_in_relation(api.Relation.create(schema='bdd_prod_de', identifier='wp_jb_partial_cancelations')) -%}
{%- set es_columns = adapter.get_columns_in_relation(api.Relation.create(schema='bdd_prod_es', identifier='wp_jb_partial_cancelations')) -%}
{%- set it_columns = adapter.get_columns_in_relation(api.Relation.create(schema='bdd_prod_it', identifier='wp_jb_partial_cancelations')) -%}
...
FROM `bdd_prod_fr.wp_jb_partial_cancelations` t
WHERE 
...
FROM `bdd_prod_de.wp_jb_partial_cancelations` t
WHERE 
...
FROM `bdd_prod_es.wp_jb_partial_cancelations` t
WHERE 
...
FROM `bdd_prod_it.wp_jb_partial_cancelations` t
WHERE 
