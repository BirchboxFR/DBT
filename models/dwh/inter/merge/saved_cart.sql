{%- set fr_columns = adapter.get_columns_in_relation(api.Relation.create(schema='bdd_prod_fr', identifier='wp_jb_saved_cart')) -%}
{%- set de_columns = adapter.get_columns_in_relation(api.Relation.create(schema='bdd_prod_de', identifier='wp_jb_saved_cart')) -%}
...
FROM `bdd_prod_fr.wp_jb_saved_cart` t
WHERE 
...
FROM `bdd_prod_de.wp_jb_saved_cart` t
WHERE 
...
FROM `bdd_prod_es.wp_jb_saved_cart` t
WHERE 
...
FROM `bdd_prod_it.wp_jb_saved_cart` t
WHERE 
