...
{%- for country in countries -%}
  {%- set delete_sql -%}
DELETE FROM `teamdata-291012.{{ country.dataset }}.wp_jb_survey_answer_meanings` 
WHERE (id) IN (
  SELECT CAST(JSON_EXTRACT_SCALAR(_airbyte_data, '$.id') AS INT64)
  FROM `teamdata-291012.airbyte_internal.{{ country.dataset }}_raw__stream_wp_jb_survey_answer_meanings`
...
SELECT 
  ...
  b.*
FROM `teamdata-291012.{{ country.dataset }}.wp_jb_survey_answer_meanings` b
