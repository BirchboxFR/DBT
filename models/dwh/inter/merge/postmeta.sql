...
{%- for country in countries -%}
  {%- set delete_sql -%}
DELETE FROM `teamdata-291012.{{ country.dataset }}.wp_postmeta` 
WHERE (meta_id) IN (
  SELECT CAST(JSON_EXTRACT_SCALAR(_airbyte_data, '$.id') AS INT64)
  FROM `teamdata-291012.airbyte_internal.{{ country.dataset }}_raw__stream_wp_postmeta`
...
{%- for country in countries %}
SELECT 
  '{{ country.code }}' as dw_country_code,meta_id as id,
  b.*
FROM `teamdata-291012.{{ country.dataset }}.wp_postmeta` b
