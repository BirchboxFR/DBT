{%- set fr_columns = adapter.get_columns_in_relation(api.Relation.create(schema='bdd_prod_fr', identifier='wp_jb_b2c_order_notifications')) -%}
{%- set de_columns = adapter.get_columns_in_relation(api.Relation.create(schema='bdd_prod_de', identifier='wp_jb_b2c_order_notifications')) -%}
{%- set es_columns = adapter.get_columns_in_relation(api.Relation.create(schema='bdd_prod_es', identifier='wp_jb_b2c_order_notifications')) -%}
{%- set it_columns = adapter.get_columns_in_relation(api.Relation.create(schema='bdd_prod_it', identifier='wp_jb_b2c_order_notifications')) -%}
...
) , safe_cast(event_date as datetime) event_Date
FROM `bdd_prod_fr.wp_jb_b2c_order_notifications` t
...
FROM `bdd_prod_de.wp_jb_b2c_order_notifications` t
...
FROM `bdd_prod_es.wp_jb_b2c_order_notifications` t
...
FROM `bdd_prod_it.wp_jb_b2c_order_notifications` t
