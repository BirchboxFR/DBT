...
ac.dw_country_code,
      ...
      END AS optin_active_email,
       ...
       ud.optin_sms,
       ud.optin_whatsapp,
       coalesce(ud.is_admin,FALSE) is_admin,
       ud.firstname,
       ud.lastname,
       ip.gender,
      ...
      bpt.self_taining, 
          ...
          bpt.waxing_razor,
          bpt.waxing_depilatory_cream,
          bpt.electric_epilator,
          bpt.home_waxing,
          bpt.salon_laser_waxing,
          bpt.no_waxing,
          bpt.no_waxing_response,
       sst.box_sub_status,
...
LEFT JOIN (SELECT address, status, eventdate
  FROM (
    SELECT address, status, eventdate, ROW_NUMBER() OVER ( PARTITION BY address ORDER BY eventdate DESC,
          CASE status
    ...
    FROM `teamdata-291012.cdpimagino.BQ_imagino_Address_Status`
  )
  WHERE rn = 1
) b ON ac.email = b.address